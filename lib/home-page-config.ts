...
export function applyHeroBannerToSlide(
  ...
  if (
    ...
    index < 0 ||
    index >= settings.sections.hero.slides.length
  ) {
    return settings;
  }
  return {
    ...
    sections: {
      ...
      hero: {
        ...
        slides: settings.sections.hero.slides.map((slide, slideIndex) =>
          slideIndex === index ? { ...slide, imageSrc, alt } : slide,
        ),
      },
    },
  };
