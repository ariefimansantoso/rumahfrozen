...
export async function sanitizeGeneratedHtml(value: string): Promise<string> {
...
async function normalizeFields(
  ...
  for (const [key, value] of Object.entries(fields)) {
    ...
    normalized[key] = html ? await sanitizeGeneratedHtml(clipped) : clipped.trim();
  }
...
export async function normalizeContentDraft(
  request: AIAuthoringRequest,
  raw: unknown,
): Promise<AIAuthoringContentResponse> {
  const source = isPlainObject(raw) ? raw : {};
  const fields = await normalizeFields(request, collectModelFields(request, source));
...
export async function generateAuthoringContent(
  ...
  return await normalizeContentDraft(request, parsed);
}
