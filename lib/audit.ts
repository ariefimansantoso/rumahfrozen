...
export async function auditStatusChange(
  ...
  return audit(context, {
    ...
    changes: {
      ...
      fields: ["status"],
      summary: `Changed ${resource} status from "${oldStatus}" to "${newStatus}"`,
    },
  });
}
...
export async function auditLogin(
  ...
  return audit(context, {
    ...
    resourceName: context.userEmail,
    success,
    errorMessage,
    changes: {
      summary: success
...
export async function auditOrderStatusChange(
  ...
  return audit(context, {
    ...
    resource: "order",
    resourceId: orderId,
    resourceName: orderNumber ? `Order #${orderNumber}` : undefined,
    changes: {
      before: { status: oldStatus },
      after: { status: newStatus },
      fields: ["status"],
      summary: `Order status changed from "${oldStatus}" to "${newStatus}"`,
    },
  });
}
