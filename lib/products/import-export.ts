...
async function resolveCategory(row: ProductCsvRow) {
  ...
  return byName ? String(byName._id) : undefined;
}
...
function buildProductPatch(
  ...
  const patch: Record<string, unknown> = {
    ...
    barcodeFormat,
    barcodeSource,
    stock: parseRequiredNumber(row.stock, existing?.stock),
    ...
    shipping: {
      isPhysicalProduct: parseBoolean(row.isPhysicalProduct, true),
      weight: parseOptionalNumber(row.weight),
      weightUnit: row.weightUnit || row.weight_unit || "kg",
      countryOfOrigin: row.countryOfOrigin || row.country_of_origin || undefined,
...
export async function importProductsCsv(
  ...
  for (let index = 0; index < rows.length; index++) {
    ...
    try {
      ...
      const categoryId = await resolveCategory(row);
