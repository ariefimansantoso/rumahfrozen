...
function sanitizeLocationInventory(raw: unknown): Loose[] | undefined {
 ...
 */
export function sanitizeVariantsForMongoose(input: unknown): Loose[] {
  ...
  for (const raw of input) {
    ...
    const li = sanitizeLocationInventory(variant.locationInventory);
 ...
 * entries; drops anything missing locationId.
 */
export function sanitizeProductLocationInventory(
  input: unknown,
): Loose[] | undefined {
  return sanitizeLocationInventory(input);
}
...
export function sanitizeOptionsForMongoose(input: unknown): Loose[] {
  ...
  for (const raw of input) {
    ...
    const values = Array.isArray(raw.values)
      ...
      : [];
    out.push({ _id, name, position, values });
  }
  return out;
}
