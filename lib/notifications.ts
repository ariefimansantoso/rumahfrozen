  ...
  return createConfiguredNotification({
    ...
    message:
      status === "approved"
        ...
        : "Your vendor application needs some updates. Please check your application.",
    link: status === "approved" ? "/vendor/dashboard" : "/become-vendor",
    data: { status },
  }, channels);
}
...
export async function notifyReturnRequestSubmitted(
  ...
  try {
    ...
    if (returnRequest.ownerType === "vendor") {
      ...
      await notifyReturnRequestOwnerUser(
        vendorUser,
        returnRequest,
        USER_ROLES.VENDOR,
        settings,
