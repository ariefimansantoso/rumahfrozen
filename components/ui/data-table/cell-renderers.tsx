...
export function CurrencyCell({ value, currency }: CurrencyCellProps) {
  const { currency: currentCurrency } = useCurrency();
  if (value == null) return <span className="text-muted-foreground">—</span>;

  const displayCode = currency ?? currentCurrency.code;
  const displayLocale = currentCurrency.locale;
  const formatted = new Intl.NumberFormat(displayLocale, {
    style: "currency",
    currency: displayCode.toUpperCase(),
  }).format(value);
