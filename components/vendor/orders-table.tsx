...
export function VendorOrdersTable({
  ...
  const list = useListQuery<VendorOrder>({
    ...
    initialSortBy,
    initialSortOrder,
    initialFilters: { status: initialStatus, paymentStatus: initialPaymentStatus },
    errorMessage: t("vendor.ordersTable.loadFailed"),
  });
      ...
      try {
        const results = await Promise.all(
          items.map(async (order) => {
            ...
            if (!subOrder || !isAllowedTransition(subOrder.status, newStatus)) {
              return false;
            }
