...
export function VendorOrderDetails({
  ...
  const [refundDialog, setRefundDialog] = useState<VendorReturnRequest | null>(null);
  ...
  const refundableReturns = returnRequests.filter((request) =>
  ...
  const canStartProcessing =
    canEditOrder && isAllowedTransition(subOrder.status, "processing");
  const canMarkShipped =
    canEditOrder && isAllowedTransition(subOrder.status, "shipped");
  const canMarkDelivered =
    canEditOrder && isAllowedTransition(subOrder.status, "delivered");
  const canCancel =
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {refundDialog?.returnNumber} for order {order.orderNumber}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2 py-2">
            <Label htmlFor="vendor-return-refund-amount">Refund amount</Label>
            <Input
              id="vendor-return-refund-amount"
              type="number"
              min={0}
              step="0.01"
              value={refundAmount}
          ...
          <DialogFooter>
            ...
            <Button
              disabled={isUpdating || !refundDialog}
              onClick={() => {
                if (!refundDialog) return;
                void updateReturn(refundDialog, {
                  status: "refunded",
