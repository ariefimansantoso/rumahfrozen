...
interface CustomerInboxContentProps {
  locale: string;
...
export function CustomerInboxContent({
  locale,
  initialConversations,
  initialSelectedConversationId,
}: CustomerInboxContentProps) {
  ...
  const submitReply = async () => {
    ...
    try {
      const response = await fetch(
        `/api/account/inbox/${selectedConversation._id}/reply`,
        {
          method: "POST",
  ...
  if (conversations.length === 0) {
    return (
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-3 rounded-xl border bg-card px-6 text-center shadow-sm">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {conversations.length} total &middot; {openCount} open
              </p>
            </div>
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    ...
                    {selectedConversation.sender.status && (
                      <Badge
                        variant="outline"
                    ...
                    >
                      {showMeta ? (
                        <Avatar className="h-9 w-9 shrink-0">
                          ...
                          />
                          <AvatarFallback className="text-xs font-semibold">
                            {fromAdmin ? "AD" : getInitials(message.senderName)}
                          </AvatarFallback>
                        </Avatar>
                      ) : (
