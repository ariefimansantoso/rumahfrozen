...
export function CustomerNotifications({ locale }: CustomerNotificationsProps) {
  ...
  const requestNotifications = useCallback(async (tab: TabType) => {
    const response = await fetch(`/api/notifications?tab=${tab}&limit=50`, {
      cache: "no-store",
    });
    ...
    return {
      notifications: (json.data?.notifications || []) as NotificationRecord[],
      counts: (json.data?.counts || initialCounts) as NotificationCounts,
    };
  }, []);
  ...
  const loadNotifications = useCallback(
    async (tab: TabType = activeTab) => {
      try {
        const snapshot = await requestNotifications(tab);
    ...
    [activeTab, requestNotifications],
  );
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const snapshot = await requestNotifications(activeTab);
        if (!isActive) return;
        setNotifications(snapshot.notifications);
        setCounts(snapshot.counts);
      } catch (error) {
