...
function getInitialSortColumn(value: string | null) {
  return value === "grossAmount" || value === "netAmount" || value === "createdAt"
    ? value
    : "createdAt";
}

export function PaymentTransactionsTable({ locale }: { locale: string }) {
  const t = useTranslations();
  const router = useRouter();
  const searchParams = useSearchParams();
  const list = useListQuery<TransactionRow>({
    endpoint: "/api/admin/payments/transactions",
    initialPage: parsePositiveInt(searchParams.get("page"), 1),
    initialPageSize: parsePositiveInt(searchParams.get("limit"), 20),
    initialSearch: searchParams.get("search") || "",
    initialTab: searchParams.get("status") || "all",
    initialSortBy: getInitialSortColumn(searchParams.get("sortBy")),
  ...
  const columns = useMemo<DataTableColumn<TransactionRow>[]>(
    () => [
      ...
      },
      {
        ...
        cell: (row) => (
          <TextCell
            value={formatCurrency(row.grossAmount, row.currency || "USD", locale)}
      ...
      },
      {
        ...
        cell: (row) => (
          <TextCell
            value={formatCurrency(row.netAmount, row.currency || "USD", locale)}
            className="block w-full text-right font-medium"
          />
        ),
        sortable: true,
    ...
    [formatDateTime, locale, t, translateProvider, translateStatus, translateType],
  );
    ...
    <div className="space-y-4">
      ...
      <DataTable
        ...
        onPageSizeChange={(pageSize) => {
          list.handlePageSizeChange(pageSize);
          list.updateUrlParams({ limit: String(pageSize) });
        }}
