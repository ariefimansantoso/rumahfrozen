...
export function AdminPayoutsContent({
  ...
  const list = useListQuery<PayoutTableRow>({
    ...
    initialSortBy,
    initialSortOrder,
    initialFilters: { vendorId: initialVendorId },
  ...
  useEffect(() => {
    ...
    const loadVendors = async () => {
      try {
        const data = await apiClient.get<{ data?: VendorOption[] }>(
          "/api/admin/vendors",
          { query: { page: 1, limit: 100 } },
        );
        if (cancelled) return;
        const rows = (data?.data || []) as Array<{ _id: string; storeName: string }>;
        setVendors(rows.map((v) => ({ _id: v._id, storeName: v.storeName })));
      } catch {
        // Vendor filter options are non-critical; leave the list empty.
      }
    };
    void loadVendors();
    return () => {
