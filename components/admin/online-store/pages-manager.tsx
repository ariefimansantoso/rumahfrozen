    ...
    <div className="flex items-center gap-0.5">
      <Tooltip>
        <TooltipTrigger asChild>
          ...
          >
            <Link href={entry.editPath} aria-label={`Edit ${entry.title}`}>
        ...
        <TooltipTrigger asChild>
          ...
          >
            <Link
              ...
              target="_blank"
              aria-label={`Open ${entry.title} on storefront`}
            >
              <ExternalLink className="h-4 w-4" />
            </Link>
          </Button>
        </TooltipTrigger>
        <TooltipContent>Open on storefront</TooltipContent>
      </Tooltip>
      ...
      <TooltipTrigger asChild>
        <span className="inline-flex">
          <Switch
            ...
            disabled={entry.saving}
            aria-label={`Toggle ${entry.title} visibility`}
          />
        </span>
      </TooltipTrigger>
      <TooltipContent>
        {entry.visible ? "Hide from storefront" : "Show on storefront"}
      </TooltipContent>
    </Tooltip>
  );
}
      ...
      >
        ...
        <div className="min-w-0">
          ...
          <p className="mt-0.5 truncate text-xs leading-tight text-muted-foreground">
            {entry.description}
            {entry.updatedAt ? (
              <span> &middot; Updated {formatUpdatedAt(entry.updatedAt)}</span>
            ) : null}
          </p>
        </div>
      </Link>
...
export function PagesManager({
  ...
  const toggleBuiltInVisibility = async (
    ...
    const nextLabel = nextVisible ? "visible" : "hidden";
    ...
    try {
      ...
      await saveContentPages(
        ...
        `${pageMeta.adminTitle} is now ${nextLabel}.`,
      );
    } catch {
      toast.error(`Failed to update ${pageMeta.adminTitle} visibility`);
    } finally {
  ...
  const toggleCustomVisibility = async (pageId: string) => {
    ...
    try {
      ...
      await saveContentPages(
        ...
        `"${targetPage.title}" is now ${nextVisible ? "visible" : "hidden"}.`,
      );
    } catch {
      toast.error(`Failed to update "${targetPage.title}" visibility`);
    } finally {
      setSaving(savingKey, false);
    }
  };
  ...
  const deleteCustomPage = async (pageId: string) => {
    ...
    try {
      ...
      toast.error(`Failed to delete "${page.title}"`);
    } finally {
      setSaving(savingKey, false);
    }
  };
    ...
    () =>
      [...contentPages.customPages]
        ...
        .map((page) => ({
          ...
          title: page.title,
          description: page.metaDescription || "Custom storefront page.",
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground md:text-base">
              Manage informational storefront pages.
            </p>
            ...
            {customEntries.length === 0 ? (
              <div className="rounded-xl border border-dashed border-border bg-card/50 px-6 py-12 text-center">
                ...
                <p className="mx-auto mt-1 max-w-sm text-xs leading-relaxed text-muted-foreground">
                  Create landing pages, guides, or any informational content
                  with their own storefront URL.
                </p>
              ...
              <p className="text-sm text-muted-foreground">
                No custom pages match your filters.
              </p>
            )}
          </section>
        </>
