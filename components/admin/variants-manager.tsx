...
import {
  GlobalVariantPicker,
  type GlobalVariantOption,
} from "@/components/admin/variants-manager/global-variant-picker";
...
export function VariantsManager({
  ...
  // Attach a pre-built global variant as a product option, copying its values
  // (and colour codes) so the existing variant-generation machinery treats it
  // exactly like a manually-typed option.
  const handleAddGlobalVariant = (variant: GlobalVariantOption) => {
    if (options.length >= MAX_OPTION_COUNT) return;
      ...
      <div className="space-y-3">
        ...
        {options.length < MAX_OPTION_COUNT && (
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2">
            ...
            <GlobalVariantPicker
              existingOptionNames={options.map((option) => option.name)}
              onSelect={handleAddGlobalVariant}
            />
          </div>
        )}
      </div>
      ...
      {hasValidOptions && variants.length > 0 && (
        <>
          ...
          {selectedVariants.size > 0 && (
            <div className="flex flex-wrap items-center gap-3 py-2 px-3 bg-primary/10 border border-primary/20 rounded-lg">
