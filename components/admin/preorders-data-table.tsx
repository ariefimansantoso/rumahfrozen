...
export function PreordersDataTable({
  ...
  const list = useListQuery<PreorderOrder>({
    endpoint: `/api/${scope}/preorders`,
    initialPage,
    initialSearch,
    initialTab: initialStatus,
    // "due_soon"/"overdue" are views, not statuses; there is no sort UI.
    mapQuery: ({ sortBy: _sortBy, sortOrder: _sortOrder, status, ...rest }) => ({
      ...rest,
    ...
    errorMessage: "Failed to load pre-orders",
  });
    ...
    () => [
      ...
      },
      {
        ...
        cell: (row) => {
          ...
          return (
            <Badge className={getPreorderStatusStyles(status)}>
      ...
      },
      {
        ...
        headerClassName: "!px-4",
        cell: (row) => (
          <Badge className={getOrderStatusStyles(row.status)}>
      ...
      },
      {
        ...
        headerClassName: "!px-4",
        cell: (row) => (
          <Badge className={getPaymentStatusStyles(row.paymentStatus)}>
  ...
  return (
    <>
      <DataTable
        ...
        className="[&_table]:w-full [&_table]:table-fixed [&_th:last-child]:!w-[74px] [&_th:last-child]:!px-4 [&_td:last-child]:!px-4"
        dense
      />
