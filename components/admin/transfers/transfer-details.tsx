...
export function TransferDetails({ locale, transferId }: { locale: string; transferId: string }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (json.success) {
        setRecord(json.data?.transfer || null);
      } else {
        throw new Error(json.message || t("admin.transfers.details.toasts.loadError"));
      }
    } catch (error) {
      toast.error(
  ...
  const updateStatus = async (status: TransferStatus) => {
    ...
    setUpdating(true);
    try {
      const res = await fetch(`/api/admin/transfers/${record._id}`, {
        method: "PATCH",
      ...
      const json = await res.json();
      if (!json.success) {
        throw new Error(json.message || t("admin.transfers.details.toasts.updateError"));
      }
  ...
  if (loading) {
    return (
      <div className="py-16 text-center text-muted-foreground">
        ...
        {t("admin.transfers.details.loading")}
      </div>
    );
  }
