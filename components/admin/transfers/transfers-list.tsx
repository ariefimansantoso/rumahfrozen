...
export function TransfersList({ locale }: { locale: string }) {
  const t = useTranslations();
  const router = useRouter();
  const searchParams = useSearchParams();
  const initialPageParam = parseInt(searchParams.get("page") || "1", 10);

  const list = useListQuery<TransferRow>({
    ...
    initialSortBy: searchParams.get("sortBy") || "createdAt",
  ...
  const counters = useMemo(
    () => ((list.rawData as ApiPayload | undefined)?.counters) || {},
    [list.rawData],
  );
