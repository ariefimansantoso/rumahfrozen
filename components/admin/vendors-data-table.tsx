...
export function VendorsDataTable({
  ...
  const list = useListQuery<Vendor>({
    endpoint: "/api/admin/vendors",
    initialPage,
    initialSearch,
    initialTab: initialStatus,
    // This table has no sort UI; keep the request shape the API expects.
    mapQuery: ({ sortBy: _sortBy, sortOrder: _sortOrder, ...rest }) => rest,
    errorMessage: tr("Failed to fetch vendors", "ভেন্ডর লোড করা যায়নি"),
  });
        ...
        cell: (row) => (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="truncate font-medium">{row.storeName}</p>
  ...
  const rowActions = useCallback(
    (row: Vendor): DataTableAction[] => {
      ...
      if (row.status !== VENDOR_STATUS.APPROVED) {
        actionsList.push({
          id: "approve",
          label: tr("Approve vendor", "ভেন্ডর অনুমোদন"),
