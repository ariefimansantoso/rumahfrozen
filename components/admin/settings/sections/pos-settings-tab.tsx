      ...
      <div className="space-y-6">
        ...
        >
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <ToggleRow
                icon={Receipt}
                label={t("admin.settings.pos.customerReceiptSelection")}
                description={t(
                  "admin.settings.pos.customerReceiptSelectionDesc"
                )}
                checked={Boolean(
                  pos.checkout?.customerReceiptSelectionEnabled
                )}
                onChange={(v) =>
                  props.updateField(
                    "pos.checkout.customerReceiptSelectionEnabled",
        ...
        >
          <div className="space-y-2">
            <ToggleRow
              icon={LayoutGrid}
              label={t("admin.settings.pos.smartGrid")}
              description={t("admin.settings.pos.smartGridDesc")}
              checked={Boolean(pos.customize?.smartGridEnabled)}
              onChange={(v) =>
                props.updateField("pos.customize.smartGridEnabled", v)
              }
              disabled={!pos.enabled}
            />
            <ToggleRow
            ...
            <ToggleRow
              icon={Monitor}
              label={t("admin.settings.pos.customerDisplays")}
              description={t("admin.settings.pos.customerDisplaysDesc")}
              checked={Boolean(pos.customize?.customerDisplayEnabled)}
              onChange={(v) =>
                props.updateField("pos.customize.customerDisplayEnabled", v)
              }
              disabled={!pos.enabled}
            />
          </div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-2">
              <Label htmlFor="lockScreenTimeout" className="flex items-center gap-2">
                <Lock className="h-3.5 w-3.5 text-muted-foreground" />
                {t("admin.settings.pos.lockScreen")}
              </Label>
              <div className="flex items-center gap-2">
                <Input
                  ...
                  max={60}
                  value={String(
                    pos.customize?.lockScreenTimeoutMinutes ?? 5
                  )}
                  onChange={(e) =>
                    props.updateField(
                      "pos.customize.lockScreenTimeoutMinutes",
                ...
                <span className="text-sm text-muted-foreground">
                  {t("admin.settings.pos.minutes")}
                </span>
              </div>
              <p className="text-xs text-muted-foreground">
                {t("admin.settings.pos.lockScreenHint")}
              </p>
            </div>
        ...
        >
          ...
          <ToggleRow
            icon={RotateCcw}
            label={t("admin.settings.pos.returnRules")}
            description={t("admin.settings.pos.returnRulesDesc")}
            checked={Boolean(pos.orders?.returnRules?.enabled)}
            onChange={(v) =>
              props.updateField("pos.orders.returnRules.enabled", v)
            }
            disabled={!pos.enabled}
          />

          {pos.orders?.returnRules?.enabled && (
            <div className="space-y-4 rounded-lg border bg-muted/30 p-4 animate-in fade-in slide-in-from-top-1 duration-200">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-2">
                  <Label htmlFor="returnWindowDays" className="flex items-center gap-2">
                    <Timer className="h-3.5 w-3.5 text-muted-foreground" />
                    {t("admin.settings.pos.returnWindowDays")}
                  </Label>
                  <div className="flex items-center gap-2">
                    <Input
                      id="returnWindowDays"
                      ...
                      max={365}
                      value={String(
                        pos.orders?.returnRules?.windowDays ?? 30
                      )}
                      onChange={(e) =>
                        props.updateField(
                          "pos.orders.returnRules.windowDays",
                          Number(e.target.value)
                        )
                      }
                      disabled={!pos.enabled}
              ...
              <ToggleRow
                label={t("admin.settings.pos.allowWithoutReceipt")}
                description={t(
                  "admin.settings.pos.allowWithoutReceiptDesc"
                )}
                checked={Boolean(
                  pos.orders?.returnRules?.allowWithoutReceipt
                )}
                onChange={(v) =>
                  props.updateField(
                    "pos.orders.returnRules.allowWithoutReceipt",
              ...
              <div className="space-y-2">
                <Label htmlFor="conditionNotes">
                  {t("admin.settings.pos.conditionNotes")}
                </Label>
                <Textarea
                  id="conditionNotes"
                  placeholder={t(
                    "admin.settings.pos.conditionNotesPlaceholder"
                  )}
                  value={pos.orders?.returnRules?.conditionNotes || ""}
                  onChange={(e) =>
                    props.updateField(
                      "pos.orders.returnRules.conditionNotes",
                      e.target.value
                    )
                  }
                  disabled={!pos.enabled}
                ...
                />
                <p className="text-xs text-muted-foreground">
                  {t("admin.settings.pos.conditionNotesHint")}
                </p>
              </div>
            </div>
          )}
