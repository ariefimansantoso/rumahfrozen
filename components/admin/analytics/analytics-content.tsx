...
function getRangeLengthDays(range: AppliedDateRange): number {
  const msPerDay = 24 * 60 * 60 * 1000;
  return Math.max(
    1,
    Math.round((startOfDay(range.to).getTime() - startOfDay(range.from).getTime()) / msPerDay) + 1,
  );
}

function getChartTickPeriod(period: Period, range: AppliedDateRange): Period {
  if (period !== "custom") return period;
  const days = getRangeLengthDays(range);
...
function getPeriodLabel(value: Period, t: ReturnType<typeof useTranslations>): string {
  if (value === "custom") {
  ...
  return (
    <Popover open={open} onOpenChange={handleOpenChange}>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className="h-9 w-full max-w-full justify-start gap-1.5 rounded-sm border-[#dfe5ee] bg-card px-3 text-xs font-medium text-[#172033] shadow-xs hover:bg-card hover:text-[#172033] sm:w-55 dark:border-border dark:text-foreground dark:hover:bg-card dark:hover:text-foreground"
        >
          <CalendarDays className="size-3.5 text-[#172033] dark:text-muted-foreground" />
          <span className="min-w-0 flex-1 truncate text-left">
            {formatAppliedDateRange(value, locale)}
          </span>
          <ChevronDown className="size-3.5 text-[#172033] dark:text-muted-foreground" />
        </Button>
      </PopoverTrigger>
      <PopoverContent
        align="start"
        sideOffset={10}
        className="w-[calc(100vw-2rem)] max-w-[636px] overflow-hidden !rounded-sm border-[#dfe5ee] bg-card p-0 shadow-[0_14px_32px_rgba(16,24,40,0.14)] dark:border-border"
      >
        ...
        <div className="flex items-center justify-end gap-2 border-t border-[#dfe5ee] px-4 py-3 dark:border-border">
          <Button
            type="button"
            variant="outline"
            size="sm"
            onClick={() => setOpen(false)}
            className="h-9 rounded-lg border-[#dfe5ee] px-4 text-[13px] font-medium text-[#172033] shadow-xs dark:border-border dark:text-foreground"
          >
          ...
          <Button
            ...
            onClick={handleApply}
            className="h-9 rounded-lg bg-[#1d5cff] px-4 text-[13px] font-semibold text-white shadow-none hover:bg-[#174ee0]"
          >
            {tFallback(t, "common.apply", "Apply")}
          </Button>
        </div>
      </PopoverContent>
    </Popover>
...
function AnalyticsSkeleton() {
  return (
    <>
      ...
      <DashboardStatsGridSkeleton
        items={6}
        className="grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6"
      />
...
export function AdminAnalyticsContent({
  ...
  const fetchAll = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (isConfigured) {
          if (aggData?.data?.results)
            setAggregate(aggData.data.results);
          if (tsData?.data?.results)
            setTimeseries(tsData.data.results);
          if (pagesData?.data?.results)
            setTopPages(pagesData.data.results);
  ...
  const trafficStats: DashboardStatCardItem[] = [
    ...
    },
    {
      ...
      icon: <Clock />,
      value: aggregate ? formatDuration(aggregate.visit_duration.value, t) : "—",
    },
  ];
                ...
                <BreakdownTable
                  ...
                  labelRender={(row) => (
                    <span className="flex items-center gap-2">
                      ...
                      {row.source ||
                        t("admin.analyticsPage.directNone")}
                    </span>
                  )}
                  loading={loading}
