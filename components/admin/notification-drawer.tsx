...
interface NotificationStreamSnapshot {
...
export function NotificationDrawer({ locale }: NotificationDrawerProps) {
  ...
  const streamBootstrappedRef = useRef(false);
  ...
  const applyNotificationSnapshot = useCallback(
    (
      snapshot: NotificationStreamSnapshot,
      ...
      if (
        ...
        streamBootstrappedRef.current &&
        newNotifications.length > 0
      ) {
        showRealtimeToast(newNotifications[0], newNotifications.length);
      }

      streamBootstrappedRef.current = true;
    },
    [rememberNotificationIds, showRealtimeToast],
  );

  const fetchNotifications = useCallback(
    async (tab: TabType = activeTab, showLoading = true) => {
      if (typeof fetch !== "function") return;

      if (showLoading) setIsFetching(true);
      try {
        const response = await fetch(`/api/notifications?tab=${tab}&limit=20`, {
          cache: "no-store",
        });
        ...
        if (json?.success) {
          const nextNotifications = json.data?.notifications || [];
          rememberNotificationIds(nextNotifications);
          setNotifications(nextNotifications);
          setCounts(json.data?.counts || initialCounts);
        }
      } catch {
        // The drawer refreshes again on the next poll/open.
      } finally {
        if (showLoading) setIsFetching(false);
      }
    },
    [activeTab, rememberNotificationIds],
  );
  ...
  useEffect(() => {
    ...
    if (!EventSourceConstructor) {
      void fetchNotifications(activeTab, false);
    ...
    streamBootstrappedRef.current = false;
    const eventSource = new EventSourceConstructor(
      `/api/notifications/stream?tab=${activeTab}&limit=20`,
    );
    ...
    const handleSnapshot = (event: Event) => {
      try {
        ...
        // The next stream event contains a complete snapshot.
      }
    };
  ...
  }, [activeTab, applyNotificationSnapshot, fetchNotifications]);

  useEffect(() => {
    if (!open) return;
    void fetchNotifications(activeTab);
  }, [activeTab, fetchNotifications, open]);
          ...
          >
            ...
            <div className="flex justify-center px-4 py-4 sm:px-5">
              <Button
                ...
                onClick={() => void fetchNotifications(activeTab)}
              >
                Refresh
              </Button>
            </div>
