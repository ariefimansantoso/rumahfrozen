...
export function BlogPostsDataTable({
  ...
  }, [pagination.page, pagination.pageSize, searchValue, activeTab, sortColumn, sortDirection]);
  ...
  const handleBulkDelete = useCallback(
    async (items: BlogPost[]) => {
      const ok = await confirm({
        ...
        description: `${items.length} post(s) will be permanently removed.`,
        confirmText: "Delete",
        cancelText: "Cancel",
        variant: "destructive",
      });
      ...
      toast.success(`Deleted ${items.length} post(s)`);
      setSelected([]);
      fetchPosts();
    },
    [confirm, fetchPosts],
  ...
  const handleBulkStatus = useCallback(
    async (items: BlogPost[], status: string) => {
      ...
      setSelected([]);
      fetchPosts();
      if (ok > 0) toast.success(`Updated ${ok} post(s)`);
      if (ok < items.length) toast.error("Some updates failed");
    },
    [fetchPosts],
  );
  ...
  const rowActions = useCallback(
    (row: BlogPost): DataTableAction[] => [
      ...
      },
      {
        ...
        label: row.isFeatured ? "Remove featured" : "Mark featured",
    ...
    [locale, handleDelete, handleFeature],
  );
