...
export default async function AdminDiscountsPage({
  ...
  return (
    <div className="space-y-4">
      <AdminStatsStrip items={statItems} columnsClassName="lg:grid-cols-4" />
      <DiscountsContent
        locale={locale}
        initialPage={page}
  ...
  const [stats] = await Coupon.aggregate([
    {
      $facet: {
        ...
        activeCoupons: [{ $match: { status: "active", endDate: { $gte: now } } }, { $count: "count" }],
        totalRedemptions: [{ $group: { _id: null, total: { $sum: "$usedCount" } } }],
        expiringSoon: [
          { $match: { status: "active", endDate: { $gte: now, $lte: inSevenDays } } },
          { $count: "count" },
        ],
      },
    },
