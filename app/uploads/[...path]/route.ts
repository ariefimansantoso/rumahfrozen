 ...
 * security/cache headers next.config.ts sets for statically-served uploads
...
async function resolveFile(
  ...
  const filePath = path.resolve(BASE_DIR, segments.join("/"));
  if (!filePath.startsWith(BASE_DIR + path.sep)) return null;
